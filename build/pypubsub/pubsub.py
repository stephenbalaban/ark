...
#---------------------------------------------------------------------------
"""
This module provides a publish-subscribe object, Publisher, that allows
your methods, functions, and any other callable object to subcribe to 
...
This module is currently in use in the wxPython project (as wx.lib.pubsub), 
where it was created, but is made available on the Python 
Cheeseshop because it could be useful to a much wider audience, and is 
independent of wxPython.  Thanks to Robb Shecter and Robin Dunn for 
having provided the basis for this module.  The latest version shares 
with the original mostly the API; the implementation was completely changed 
to allow for weak referencing to the callables, so as not to 
affect their lifetime.  See Publisher class docs for 
more details. 
